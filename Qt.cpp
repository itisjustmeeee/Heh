...
{
    QString filename = QFileDialog::getSaveFileName(this, "open file", "", "test (*.txt)");
    if (filename.isEmpty()){
        QMessageBox::critical(this, "Error", "you did not choose your file");
        return;
    }
    QFile file(filename);
    ...
    QTextStream ts(&file);
    QString username, fio, gender, passport, date, phone, email;
    while (!ts.atEnd()){
        QString firstline = ts.readLine().trimmed();
        username = firstline;
    }
    while (!ts.atEnd()){
        QString secondline = ts.readLine().trimmed();
        fio = secondline;
    }
    while (!ts.atEnd()){
        QString thirdline = ts.readLine().trimmed();
        gender = thirdline;
    }
    while (!ts.atEnd()){
        QString fourthline = ts.readLine().trimmed();
        passport = fourthline;
    }
    while (!ts.atEnd()){
        QString fifthline = ts.readLine().trimmed();
        date = fifthline;
    }
    while (!ts.atEnd()){
        QString sixthline = ts.readLine().trimmed();
        phone = sixthline;
    }
    while (!ts.atEnd()){
        QString seventhline = ts.readLine().trimmed();
        email = seventhline;
    }
